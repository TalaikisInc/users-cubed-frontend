...
describe('SSR works', () => {
  ...
  it('user agents for crawlers should pass filter', async (done) => {
    crawlers.forEach((e) => {
      if (!(/google|bing|msn|duckduckbot|facebot|facebookexternalhit|slurp|yandex/i.test(e))) {
        done(`Not detected as crawler: ${e}`)
      }
    })
    done()
  })

  it('user agents for users should pass filter', async (done) => {
    userUas.forEach((e) => {
      if (/google|bing|msn|duckduckbot|facebot|facebookexternalhit|slurp|yandex/i.test(e)) {
        done(`Wrongly detected as crawler: ${e}`)
      }
    })
    done()
  })
